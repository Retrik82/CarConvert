export default function LoadingOverlay({ active, progress }) {
  ...
  return (
    <div className="absolute inset-0 z-40 flex items-center justify-center rounded-card bg-slate-950/55 backdrop-blur-sm">
      <div className="w-[80%] max-w-md rounded-2xl border border-sky-300/30 bg-slate-900/70 p-5">
        ...
        <p className="mt-2 text-right text-xs text-ink-tertiary">{Math.round(progress)}%</p>
      </div>
    </div>
  );
}
